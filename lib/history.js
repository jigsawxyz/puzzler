...
async function updateState({ migrationSet, action, pool }) {
  return Promise.all(
    migrationSet.map(migration => insertMigrationRecord({ migration, action, pool }))
  );
}
...
function parseHistory({ state }) {
  ...
  state.forEach((historicMigration) => {
    if (historicMigration.action === 'migrate') {
      appliedTransactions.push(historicMigration.migration_hash);
    } else if (historicMigration.action === 'rollback') {
      const index = appliedTransactions.findIndex(
        transaction => transaction === historicMigration.migration_hash
      );
